...
public class MouseFlightController : MonoBehaviour
{
    ...
    private void RotateRig()
    {
        if (mouseAim == null || cam == null || cameraRig == null)
            return;
        // 변수에 아무것도 없을때 해당 함수는 그냥 넘긴다.

        // Freeze the mouse aim direction when the free look key is pressed.
        // 만약 'C' 키를 눌렀을 때 마우스 에임을 잠그고 frozenDirection를 마우스에임 앞방향으로 정의
        if (Input.GetKeyDown(KeyCode.C))
        {
            isMouseAimFrozen = true;
            frozenDirection = mouseAim.forward;
        }
        //'C'키를 놓았을 때 마우스에임 잠금을 풀고 마우스에임 앞방향을 고정 방향으로 저의
        ...
        //마우스 에임을 월드좌표로 돌리는 부분
        // cam.right축 기준으로 mouseY만큼 월드좌표를 돌린다.
    ...
    {
        if (mouseAim != null)
            // 마우스 에임 좌표에다가 V3.forward x 조준거리 한 값 더한다.
